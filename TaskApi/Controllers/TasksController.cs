...
public class TasksController : ControllerBase
{
    ...
    public ActionResult<TaskItem> Update(int id, [FromBody] UpdateTaskDto dto)
    {
        ...
        if (task is null)
            return NotFound(new {Message = $"Задача с id= {id} не найденв"});
    ...
    public ActionResult<IEnumerable<TaskItem>> Search([FromQuery] string query) {
        if (string.IsNullOrWhiteSpace(query))
            return BadRequest(new {Message = "Параметр qurty не может быть пустым"});

        var results = _tasks
            .Where(t => t.Title.Contains(query, StringComparison.OrdinalIgnoreCase) || t.Description.Contains(query, StringComparison.OrdinalIgnoreCase))
            .ToList();
    ...
    public ActionResult<IEnumerable<TaskItem>> GetByPriority(string level) {
        ...
        if (!allowed.Contains(level, StringComparer.OrdinalIgnoreCase))
            return BadRequest(new{Message = "Досутпные значени low normal high"});
